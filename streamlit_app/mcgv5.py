...
if uploaded_file is not None:
    ...
    # Crop and display images with 'depth' value, then perform OCR
    st.header("Depth Detection and OCR")

    depth_class_id = [key for key, value in model.names.items() if value == 'depth']

    if depth_class_id:
        depth_class_id = depth_class_id[0]
        depth_boxes = [box for box in results[0].boxes if box.cls == depth_class_id]

        if depth_boxes:
            reader = easyocr.Reader(['en'])  # Initialize EasyOCR reader

            all_depths = []  # Collect all depths from all images

            for i, box in enumerate(depth_boxes):
                # Extract bounding box coordinates
                x1, y1, x2, y2 = map(int, box.xyxy[0])
                cropped_img = img[y1:y2, x1:x2]  # Crop the image

                # Display the cropped image
                st.image(cropped_img, caption=f"Cropped Image {i+1} (Depth)", width=400)

                # Perform OCR on the cropped image
                ocr_result = reader.readtext(cropped_img)

                # Filter OCR results to include only numbers, commas, and dots
                filtered_result = []
                for (bbox, text, confidence) in ocr_result:
                    filtered_text = re.sub(r'[^0-9.,]', '', text)  # Keep only numbers, commas, and dots
                    if filtered_text:
                        try:
                            filtered_result.append((bbox, float(filtered_text), confidence))  # Convert to FLOAT
                        except ValueError:
                            pass  # Skip if conversion fails

                # Sort results by depth value (ascending)
                sorted_result = sorted(filtered_result, key=lambda x: x[1])

                # Collect depths for final print
                depths = [text for (_, text, _) in sorted_result]
                all_depths.extend(depths)

                # Display OCR results
                st.write(f"OCR Result for Image {i+1}:")
                for (bbox, text, confidence) in sorted_result:
                    st.write(f"- Depth: {text}, Confidence: {confidence:.2f}")

            # Print all depths in ascending order from all images
            if all_depths:
                all_depths = sorted(all_depths)
                st.write(f"Depth from: {', '.join(map(str, all_depths))}")
        else:
            st.write("No 'depth' objects detected.")
    else:
        st.write("'Depth' class not found in the model.")